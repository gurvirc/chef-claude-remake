...
async function deleteRecipe(recipeId){
    try{
    const res = await fetch('http://localhost:3000/api/deleteRecipe', {
        method: 'DELETE',
        credentials: 'include',
        headers: {
            'Content-Type': 'application/json'
    ...
    if(res.ok){
        //delete the recipe from the recipe list(already deleted from db)
    ...
    return (
        ...
        <div key={recipe.id} className='recipe-card'>
            ...
            <img 
                ...
                alt={recipe.title}
            />
            <svg onClick={()=>deleteRecipe(recipe.id)} className="delete-icon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" >
